...
async function generateWindows() {
    const basePath = './files/output/builds/windows/resources';
    ...
    await mask.scan(0, 0, width, height, function (x, y, idx) {
      const dx = x < radius ? radius - x : (x  > width - radius ? x - (width - radius) : 0);
      const dy = y < radius ? radius - y : (y > height - radius ? y - (height - radius) : 0);
